 ...
 *
 * @package Valid News Pro
 */

// Recent Posts Section.
$recent_posts_section = get_theme_mod( 'valid_news_recent_posts_section_enable', false );
...
$recent_posts_content_type = get_theme_mod( 'valid_news_recent_posts_content_type', 'post' );

if ( $recent_posts_content_type === 'post' ) {

	for ( $i = 1; $i <= 3; $i++ ) {
		$content_ids[] = get_theme_mod( 'valid_news_recent_posts_post_' . $i );
	}
	...
	$cat_content_id = get_theme_mod( 'valid_news_recent_posts_category' );
	$args           = array(
		'cat'            => $cat_content_id,
		'posts_per_page' => absint( 3 ),
	);
...
if ( $query->have_posts() ) {
	...
	$section_title    = get_theme_mod( 'valid_news_recent_posts_title', __( 'Recent Articles', 'valid-news' ) );
	$view_all_btn     = get_theme_mod( 'valid_news_recent_posts_view_all_button_label', __( 'View All', 'valid-news' ) );
	$view_all_btn_url = get_theme_mod( 'valid_news_recent_posts_view_all_button_url', '#' );
	?>
